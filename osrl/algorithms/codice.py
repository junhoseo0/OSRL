...
class CODICE(nn.Module):
    ...
    def update(self, batch):
        ...
        self.nu_optim.zero_grad(set_to_none=True)
        
        # 1.2 nu forward loss
        forward_e_nu_lambda = rewards - self._lmbda.detach() * costs + self.gamma * (1.0 - done) * nu_s_next.detach() - nu_s
        forward_w_sa = F.relu(self.f_prime_inv_fn(forward_e_nu_lambda / self.alpha))
        forward_nu_loss = (1 - self.gamma) * nu_init.mean() + \
            (forward_w_sa * forward_e_nu_lambda - self.alpha * self.f_fn(forward_w_sa)).mean()
        forward_nu_loss.backward(retain_graph=True)
        forward_grads = [param.grad.clone() for param in self.nu_network.parameters()]
        self.nu_optim.zero_grad(set_to_none=True)

        # 1.3 nu backward loss
        backward_e_nu_lambda = rewards - self._lmbda.detach() * costs + self.gamma * (1.0 - done) * nu_s_next - nu_s.detach()
        backward_w_sa = F.relu(self.f_prime_inv_fn(backward_e_nu_lambda / self.alpha))
        backward_nu_loss = (1 - self.gamma) * nu_init.mean() + \
            (backward_w_sa * backward_e_nu_lambda - self.alpha * self.f_fn(backward_w_sa)).mean()
        backward_nu_loss.backward(retain_graph=True)
        backward_grads = [param.grad.clone() for param in self.nu_network.parameters()]
        self.nu_optim.zero_grad(set_to_none=True)

        # 1.4 orthogonalize backward loss.
        with torch.no_grad():
            forward_grad_flat, backward_grad_flat = [], []
            for param, forward_grad, backward_grad in zip(self.nu_network.parameters(), forward_grads, backward_grads):
                param_size = param.numel()
                forward_grad_flat.append(forward_grad.reshape(param_size))
                backward_grad_flat.append(backward_grad.reshape(param_size))
            forward_grad_flat, backward_grad_flat = torch.concat(forward_grad_flat), torch.concat(backward_grad_flat)

            projected_grad_flat = backward_grad_flat - torch.dot(forward_grad_flat, backward_grad_flat) / torch.dot(forward_grad_flat, forward_grad_flat) * forward_grad_flat
            orthogonal_grad_flat = forward_grad_flat * self.eta * projected_grad_flat

        # 1.5 Set gradients manually.
        param_idx = 0
        for param in self.nu_network.parameters():
            param_size = param.numel()
            orthogonal_grad = orthogonal_grad_flat[param_idx:param_idx+param_size].reshape(*param.shape)
            param.grad = orthogonal_grad
            param_idx += param_size
        assert param_idx == orthogonal_grad_flat.numel()
        ...
        stats_loss = {
            ...
            "loss/forward_nu_loss": forward_nu_loss.item(),
            "loss/backward_nu_loss": backward_nu_loss.item(),
